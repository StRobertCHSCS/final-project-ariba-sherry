      ...
      def check_win(self):
          ...
          for x in range(boardWidth):
              for y in range(boardHeight - 3):
                  if tile != 0 and grid[x][y] == tile and grid[x][y + 1] == tile and grid[x][y + 2] == tile and \
                          grid[x][y + 3] == tile:
                      team = (curr)
                      print("won")
                      state = 1
      ...
      def on_draw(self):
          ...
          for i in range(len(grid)):
              for j in range(len(grid[i])):
                  ...
                  if state == 0:
                      if curr == True:
                          turn = "Blue's Turn Now"
                      else:
                          turn = "Red's Turn Now"
                      arcade.draw_text(turn, 215, 135, arcade.color.WHITE, 16)
                  if state == 1:
                      if curr == True:
                          turn = "Red Won"
                          arcade.draw_text(turn, 260, 480, arcade.color.RED, 16)

                      else:
                          turn = "Blue Won"
                          arcade.draw_text(turn, 260, 480, arcade.color.BLUE, 16)
